...
int main() {
    const char *input_filename = "resultados_parciais/merge-bubble-summary_results.csv";
    const char *output_filename = "resultados_parciais/best_bubble_results.csv";
    ...
    for (int i = 0; i < tracker_count; i++) {
        ...
        fprintf(out_file, "%ld,%d,%.8f,%.8f,%.8f,%.8f,Merge+Bubble\n",
                row.tamanho,
                row.threshold,
                row.mediaCPU,
                row.desvioCPU,
